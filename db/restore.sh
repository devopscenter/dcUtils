...
if ! [[ -z "$SCHEMA_ONLY" ]]; then
    sudo -u postgres pg_restore -U postgres -s --exit-on-error -j 1 -Fc --dbname="$DB_NAME" "$LOCAL_BACKUP_FILE" || exit 1
    ...
    sudo -u postgres pg_restore -U postgres --exit-on-error -j 1 -Fc --dbname="$DB_NAME" "$LOCAL_BACKUP_FILE" || exit 1
fi
